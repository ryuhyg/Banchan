...
select * from 	
(select row_number() over(order by tr_no desc) as
rnum,tr_no,tr_quantity,tr_req_date,tr_fin_date,food_sell_no,tr_status_no,mem_id
from trade) t 
...
select r.rev_no,r.score,r.rev_postdate,r.rev_content,r.mem_id
		...
		where f.food_no='1019' and f.food_no=r.food_no and
				r.rnum between 1 and 10
		order by r.rev_postdate desc
		
select r.rev_no,r.score,r.rev_postdate,r.rev_content,r.food_no,r.mem_id
from review r, food f
where f.food_no='1019' and f.food_no=r.food_no order by r.rev_postdate desc
...
from review r, food f
where f.food_no='1019' and f.food_no=r.food_no) fr 
where fr.rnum between 16 and 20
		...
		where fs.food_sell_no='101066' and fs.food_no=r.food_no) fr 
		where fr.rnum between 1 and 5
		
		select * from food_sell where food_no='101066'
